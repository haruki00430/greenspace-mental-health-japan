...
`03_Analysis/scripts/` に抽出〜空間回帰・感度分析スクリプト、`data/processed/`・`results/` にデータ・解析出力、`04_Manuscripts/Manuscript_greenspace_mental_health.qmd` および `.html` が存在する。**下記の週次チェックリストは計画書作成時のテンプレであり、完了チェックと実フォルダが一致していない**。再開・優先度付けは本節と実ファイルを正とする。
